...
class PixelArtTextCondDiTBlock(DiTBlock):
    ...
    def forward(self, x, t, context, alpha=None, x_0=None, t_0=None, x_1=None, t_1=None, context_0=None, context_1=None, use_attn_interpolation=False):
        ...
        if use_attn_interpolation == True:
            
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
            self.scale_shift_table[None] + t.reshape(B, 6, -1)).chunk(6, dim=1)
            
            shift_msa_0, scale_msa_0, gate_msa_0, shift_mlp_0, scale_mlp_0, gate_mlp_0 = (
            self.scale_shift_table[None] + t_0.reshape(B, 6, -1)).chunk(6, dim=1)
            
            shift_msa_1, scale_msa_1, gate_msa_1, shift_mlp_1, scale_mlp_1, gate_mlp_1 = (
            self.scale_shift_table[None] + t_1.reshape(B, 6, -1)).chunk(6, dim=1)
            
            x_self_0, q_self_0, k_self_0, v_self_0 = self.attn(
                x=t2i_modulate(self.norm1(x_0), shift_msa_0, scale_msa_0))
            
            x_self_1, q_self_1, k_self_1, v_self_1 = self.attn(
                x=t2i_modulate(self.norm1(x_1), shift_msa_1, scale_msa_1))
            
            # linear
            x_self, q_self, k_self, v_self = self.attn(
                x=t2i_modulate(self.norm1(x), shift_msa, scale_msa),
                use_attn_interpolation=use_attn_interpolation,
                k=(1-alpha)*k_self_0+alpha*k_self_1,
                v=(1-alpha)*v_self_0+alpha*v_self_1)
            
            # slerp
            # x_self, q_self, k_self, v_self = self.attn(
            #     x=t2i_modulate(self.norm1(x), shift_msa, scale_msa),
            #     use_attn_interpolation=use_attn_interpolation,
            #     k=slerp(k_self_0,k_self_1,alpha),
            #     v=slerp(v_self_0,v_self_1,alpha))
            
            # fft
            # x_self, q_self, k_self, v_self = self.attn(
            #     x=t2i_modulate(self.norm1(x), shift_msa, scale_msa),
            #     use_attn_interpolation=use_attn_interpolation,
            #     k=frequency_domain_interpolation(k_self_0,k_self_1,alpha),
            #     v=frequency_domain_interpolation(v_self_0,v_self_1,alpha))
            
            
            x = x + gate_msa * x_self
            x_0 = x_0 + gate_msa_0 * x_self_0
            x_1 = x_1 + gate_msa_1 * x_self_1
      
            x_cross_0, q_cross_0, k_cross_0, v_cross_0 = self.cross_attn(x=self.prenorm_ca_text(x_0), 
                                context = self.attention_y_norm(context_0))
            x_cross_1, q_cross_1, k_cross_1, v_cross_1 = self.cross_attn(x=self.prenorm_ca_text(x_1), 
                                context = self.attention_y_norm(context_1))
            # linear
            x_cross, q_cross, k_cross, v_cross = self.cross_attn(x=self.prenorm_ca_text(x), 
                                context = self.attention_y_norm(context),
                                use_attn_interpolation=use_attn_interpolation,
                                k=(1-alpha)*k_cross_0+alpha*k_cross_1,
                                v=(1-alpha)*v_cross_0+alpha*v_cross_1)
            
            # slerp
            # x_cross, q_cross, k_cross, v_cross = self.cross_attn(x=self.prenorm_ca_text(x), 
            #                     context = self.attention_y_norm(context),
            #                     use_attn_interpolation=use_attn_interpolation,
            #                     k=slerp(k_cross_0,k_cross_1,alpha),
            #                     v=slerp(v_cross_0,v_cross_1,alpha))
            
            # fft
            # x_cross, q_cross, k_cross, v_cross = self.cross_attn(x=self.prenorm_ca_text(x), 
            #                     context = self.attention_y_norm(context),
            #                     use_attn_interpolation=use_attn_interpolation,
            #                     k=frequency_domain_interpolation(k_cross_0,k_cross_1,alpha),
            #                     v=frequency_domain_interpolation(v_cross_0,v_cross_1,alpha))
            
            # print('---------------------------------:',x_cross_0-x_cross)
            ...
            x_0 = x_0 + gate_mlp_0 * self.mlp(
                t2i_modulate(self.norm2(x_0), shift_mlp_0, scale_mlp_0))
            x_1 = x_1 + gate_mlp_1 * self.mlp(
                t2i_modulate(self.norm2(x_1), shift_mlp_1, scale_mlp_1))
