...
class TrainLoop3DRecEG3DHybridEG3DD(TrainLoop3DRecEG3D):
    ...
    def __init__(self,
        ...
        self.adv_loss_start_iter = adv_loss_start_iter
        self._prepare_nvs_pose()
        self.num_of_cano_img_for_sup = num_of_cano_img_for_sup if num_of_cano_img_for_sup > 0 else batch_size // 2 

        self.eg3d_D = D
        self.augment_pipe = augment_pipe

        self.eg3d_D.train()
        self.toggle_D_grad(True) # to avoid ddp bug
    ...
    def toggle_D_grad(self, requires_grad=False):
        # if requires_grad == False:
        #     for param in self.eg3d_D.parameters():
        ...
        for param in self.eg3d_D.parameters():
            ...
            #     # if any([res in name for res in ['b4']]):  # mimic cvD, but no performance at all on afhq.
            #     # if any([res in name for res in ('b32', 'b16', 'b8', 'b4')]): # add more parameters for gradient
            #     if any([res in name for res in ('b8', 'b4')]): # add more parameters for gradient
            #         param.requires_grad_(requires_grad)
    ...
    def forward_G_nvs(self, batch):  # update G
        ...
        for i in range(0, batch_size, self.microbatch):
            ...
            with th.autocast(device_type='cuda',
                ...
                novel_view_c = th.cat([micro['c'][1:], micro['c'][:1]], 0)
    ...
    def forward_D(self,
        ...
        for i in range(0, batch_size, self.microbatch):
            ...
            with th.autocast(device_type='cuda',
                ...
                # novel_view_c = micro['c'][self.num_of_cano_img_for_sup:][::-1] # simply reverse the order to mimic novel view
                novel_view_c = th.cat([micro['c'][self.num_of_cano_img_for_sup:][1:], micro['c'][self.num_of_cano_img_for_sup:][:1]], 0)
                ...
                cano_pred = self.rec_model(
                    latent={
                        k: v[:self.num_of_cano_img_for_sup] if isinstance(v, th.Tensor) else v for k, v in latent.items()
                    },
                ...
                log_rec3d_loss_dict(
                    {
                        'eg3d_d_loss/D_hybrid': d_loss,
                        'eg3d_d_loss/D_loss_Dreal': loss_Dreal,
                        'eg3d_d_loss/D_loss_Dgen': loss_Dgen,
                        'eg3d_d_loss/D_loss_Dr1': loss_Dr1,
                        }
                    )
    ...
    def forward_G_rec(self, batch):  # update G
        ...
        self.mp_trainer_rec.zero_grad()
        self.rec_model.requires_grad_(True)
        # self.eg3d_D.requires_grad_(False)
        self.toggle_D_grad(False)
            ...
            with th.autocast(device_type='cuda',
                ...
                with th.autocast(device_type='cuda',
                    ...
                    if self.step + self.resume_step < self.adv_loss_start_iter:
                        d_weight = th.tensor(0.0, device=dist_util.dev())
                        eg3d_d_loss = th.tensor(0.0, device=dist_util.dev())
                    else:
                        ...
                        d_weight = th.tensor(self.loss_class.opt.rec_cvD_lambda, device=dist_util.dev())
    ...
    def forward_G_hybrid(self, batch):
        ...
        for i in range(0, batch_size, self.microbatch):
            ...
            with th.autocast(device_type='cuda',
                ...
                novel_view_c = self._sample_nvs_pose(batch_size-self.num_of_cano_img_for_sup)
                ...
                if self.num_of_cano_img_for_sup > 0:
                    ...
                    target_for_rec = {
                        k:
                        v[:self.num_of_cano_img_for_sup] if v is not None else None
