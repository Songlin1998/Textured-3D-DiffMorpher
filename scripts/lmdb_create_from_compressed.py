...
def training_loop(args):
    ...
    if args.objv_dataset:
        from datasets.g_buffer_objaverse import load_data, load_eval_data, load_memory_data, load_data_for_lmdb, load_lmdb_for_lmdb
    else: # shapenet
    ...
    if args.cfg in ('afhq', 'ffhq'):
        ...
        data, dataset_size = load_lmdb_for_lmdb(
            file_path=args.data_dir,
            batch_size=args.batch_size,
            reso=args.image_size,
            reso_encoder=args.image_size_encoder,  # 224 -> 128
    ...
    def encode_and_compress_image(inp_array, is_image=False, compress=True):
        ...
        if compress:
            inp_bytes = gzip.compress(inp_bytes)
