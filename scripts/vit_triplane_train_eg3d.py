...
from nsr.script_util import create_3DAE_model, encoder_and_nsr_defaults, loss_defaults, rendering_options_defaults
...
from nsr.augment import AugmentPipe
...
def training_loop(args):
    ...
    if args.bcg_synthesis:
        args.bcg_synthesis_kwargs = G_kwargs.copy()
        args.bcg_synthesis_kwargs.update({
            'z_dim': 256,
            'w_dim': 256,
        })
        for k in ('class_name', 'rendering_kwargs', 'sr_num_fp16_res', 'sr_kwargs'):
            args.bcg_synthesis_kwargs.pop(k)
    ...
    logger.log('dataset size: {}'.format(len(training_set)))
    ...
    eval_data = th.utils.data.DataLoader(dataset=Subset(training_set, np.arange(args.batch_size)),
    ...
    TrainLoop = {
        ...
        # 'TrainLoop3DRecEG3DRealOnly': nsr.TrainLoop3DRec,   # by default
        'TrainLoop3DRecEG3DRealOnly': nsr.TrainLoop3DRecTrajVis,   # by default
        'TrainLoop3DRecEG3DRealOnl_D': nsr.TrainLoop3DRecEG3DRealOnl_D,
        'cano_nvs_cvD_eg3d': nsr.TrainLoop3DcvD_nvsD_canoD_eg3d,
    }[args.trainer_name]

    # if 'RealOnly' in args.trainer_name:

    G_ema = None
    th.cuda.empty_cache()
    gc.collect()
    logger.log('del G_ema since RealOnly training.')

    # create geometry augment pipe
    if args.geometry_augment_pipe:
        ...
        augment_pipe = AugmentPipe(**augpipe_specs['geom'])
        augment_pipe.p.copy_(th.tensor(0.4, device=dist_util.dev())) # initialize (fixed) p value
...
if __name__ == "__main__":
    ...
    # assert opts.alpha_lambda == 0, 'ffhq/afhq no alpha weight'
