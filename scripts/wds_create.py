...
def training_loop(args):
    ...
    def convert_to_lmdb(dataset_loader, lmdb_path):
        ...
        with env.begin(write=True) as txn:
            for idx, sample in enumerate(tqdm(dataset_loader)):
                ...
                sample = {
                    k:v.squeeze(0).cpu().numpy() for k, v in sample.items()
                }
            ...
            for idx, sample in enumerate(tqdm(dataset_loader)):
                ...
                for k, v in sample.items():
                    ...
                    if 'img' in k: # only bytes required? laod the 512 depth bytes only.
                        v = encode_and_compress_image(v, is_image=True)
                    # elif 'depth' in k:
                    else: # regular bytes encoding
                        v = encode_and_compress_image(v.astype(np.float32), is_image=False)
                    # else: # regular bytes encoding
                    #     v = v.tobytes()
    ...
    def convert_to_lmdb_compressed(dataset_loader, wds_path, dataset_size):
        """
        Convert a PyTorch dataset to LMDB format.
        ...
        for idx, sample in enumerate(tqdm(dataset_loader)):
            ...
            for k, v in sample.items():
                ...
                # if idx == 0: # record data shape and type for decoding
                #     sink.write(f"{k}.shape".encode("utf-8"), str(v.shape).encode("utf-8"))
                #     sink.write(f"{k}.dtype".encode("utf-8"), str(v.dtype).encode("utf-8"))
