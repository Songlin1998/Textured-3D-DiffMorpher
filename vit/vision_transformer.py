...
class MemEffAttention(Attention):
    ...
    def forward(self, x: Tensor, attn_bias=None, freqs_cis=None, use_attn_interpolation=False, k=None, v=None) -> Tensor:
        ...
        if use_attn_interpolation == False: # 只有False才需要重新计算
            ...
            if self.enable_rope:
                ...
                q, k, v = unbind(qkv, 2)
                # print('q shape: ', q.shape) # torch.Size([2, 768, 16, 64])
                # print('k shape: ', k.shape) # torch.Size([2, 768, 16, 64])
                # print('v shape: ', v.shape) # torch.Size([2, 768, 16, 64])
                q, k = self.q_norm(q), self.k_norm(k)
        else:
            if not XFORMERS_AVAILABLE:
                assert attn_bias is None, "xFormers is required for nested tensors usage"
        ...
        # if use_attn_interpolation:
        #     x = x.reshape([B, N, C+C])

        #     x = self.proj(x[...,:C])
        #     x = self.proj_drop(x)
            
        #     return x, q[...,:64], k[...,:64], v[...,:64]
        # else:
        #     x = x.reshape([B, N, C])
